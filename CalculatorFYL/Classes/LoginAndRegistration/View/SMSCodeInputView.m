...
@property (nonatomic,strong) NSMutableArray <UIImageView *> *arrayImagebg;
...
- (void)config {
    ...
    _arrayImagebg = [NSMutableArray array];
    _lastString = @"";
    ...
    _contentView.backgroundColor = rgba(238, 243, 248, 1);
...
- (void)updateSubViews {
    ...
    if (_arrayTextFidld.count < _codeCount) { //已经存在的子控件比新来的数要小, 那么就创建
        ...
        for (NSInteger i = 0; i < c; i ++) {
            ...
            UIImageView *image_icon = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"input_bg"]];
            image_icon.contentMode = UIViewContentModeScaleAspectFill;
            [_arrayImagebg addObject:image_icon];
        }
        ...
        for (NSInteger i = 0; i < c; i ++) {
            [_arrayImagebg.lastObject removeFromSuperview];
            [_arrayImagebg removeLastObject];
        }
    }
    ...
    for (NSInteger i = 0; i < _arrayTextFidld.count; i ++) {
        UIImageView * image = _arrayImagebg[i];
        [self.contentView addSubview:image];
        image.frame = CGRectMake(i * (w + _codeSpace), 0, w, self.bounds.size.height);
...
- (void)textFieldDidChangeValue:(NSNotification *)notification {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(HWTFCodeBView_endTextStr:)]) {
        [self.delegate HWTFCodeBView_endTextStr:self.lastString];
    }

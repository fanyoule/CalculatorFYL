...
- (void)handleStateChangedDisableActions:(BOOL)disable withCanSetDelegate:(BOOL)can;

// touch handling
- (CGPoint)validPointForTouch:(CGPoint)touchPoint;
- (RSColorPickerState *)stateForPoint:(CGPoint)point;
- (void)updateStateForTouchPoint:(CGPoint)point withShown:(BOOL)can;
...
- (void)initRoutine {
    ...
    [self handleStateChangedDisableActions:NO withCanSetDelegate:NO];
...
[self handleStateChangedDisableActions:YES withCanSetDelegate:NO];
...
- (void)setSelection:(CGPoint)selection {
    [self updateStateForTouchPoint:selection withShown:NO];
}
...
- (void)setCropToCircle:(BOOL)circle {
    ...
    if (circle) {
        ...
        [self updateStateForTouchPoint:point withShown:NO];
    } else {
//        [self handleStateChanged];
        [self handleStateChangedDisableActions:YES withCanSetDelegate:NO];
    }
...
- (void)handleStateChanged {
    [self handleStateChangedDisableActions:YES withCanSetDelegate:YES];
}
...
//    [self handleStateChanged];
    [self handleStateChangedDisableActions:YES withCanSetDelegate:can];
}
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    [self updateStateForTouchPoint:point withShown:YES];
